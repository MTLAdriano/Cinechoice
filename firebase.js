...
export async function fbAddFilm(uid, film) {
  const ref = await addDoc(collection(db, "users", uid, "films"), {
    ...film,
...
export async function fbBulkAddFilms(uid, films) {
  // Batch par groupes de 20 pour ne pas saturer Firestore
  const chunks = [];
  for (let i = 0; i < films.length; i += 20) chunks.push(films.slice(i, i + 20));
  for (const chunk of chunks) {
    await Promise.all(chunk.map(f =>
      addDoc(collection(db, "users", uid, "films"), { ...f, addedAt: serverTimestamp() })
    ));
  }
}
